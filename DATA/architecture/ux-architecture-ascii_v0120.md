...
# THREE-ACT LOOP · as of v0.17.0 · 2026-03-10
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  [ First Capture ]  ◀── half-screen modal · "what's here right now..."
      ...
      └── [HI] ─────────────────────────────────────────────┐        │
                          ...
                          question at bottom  ┌──────────────────┐   │
                               ...
                               └───────────────────────┤
                                                       │
                                          stored voltage routes silently
                                          classified at send · no second read
                                          null voltage (save-for-now path) → LO/FLAT · frictionless
                                                       │
                               ┌───────────────────────┤
                               │                       │
                          [HI · LIFE]          [LO · FLAT]
                               ...
                               ▼                       ▼
                     [ Seed Question ]           [ 🪵 Home ]
                     half-screen · soft hold     frictionless
                     question still open         no ceremony
                     user can answer or          origin echo
                     tap "open echoes →"         marked quietly ◀── Option C
                               │                       │
                               ▼                       │
                           [ 🪵 Home ]                 │
                           origin echo                 │
  ...
  Routing reads stored voltage from first echo · classified at send · no second read
  null voltage (save-for-now path) → LO/FLAT · frictionless · no seed question
  Routing is silent · user never sees the branch
  First live use of classifier as UX decision-maker

  Origin echo marker (Option C):
  Subtle visual distinction on first echo — quiet signal that this one
  started something. CSS class only · no UI chrome · applies both paths.

  Seed question screen (Option B · HI path only):
  Holds the seed question from lo-path modal as an input.
  User answers or navigates away. One beat before the ledger opens.
  Screen dismisses on send or on "open echoes →".
  Answer stored as second echo · same classifier · same flow · appears in list.

  Lo-path modal scope: Act I only · one-time globally.
  Act II wave/wall returns directly to 🪵 Home after analysis. No modal.
