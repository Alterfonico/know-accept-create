...
| Component                       | Watches                              | Watched by                             |
| ------------------------------- | ------------------------------------ | -------------------------------------- |
| Workers / Evaluator / Optimizer | —                                    | KW                                     |
| Kernel Witness (KW)             | Pipeline                             | Witness Prime                          |
| Witness Prime (WP)              | KW + Orchestrator monitoring actions | Nobody (irreducible)                   |
| Orchestrator                    | Pipeline + monitoring stack          | User (via `orchestrator-decisions.md`) |
| User                            | Everything                           | —                                      |
...
**False negative mitigation — canary injection:**

WP cannot observe the pipeline directly, so it cannot detect false negatives
by observation. Instead, WP uses canary injection:
...
IF kw_signal.severity == "high" OR "critical":
  → WP checks signal against KW's current signal quality metrics
...
IF WP detects Invariant 6 flag (ORCH_MONITORING_ACTION_*):
...
3. User notified: `"Meta-observer offline. Monitoring is single-layer only."`
...
1. Orchestrator logs restart reason to `orchestrator-decisions.md`
2. WP starts from deterministic initial state (this spec = its full definition)
3. WP re-runs bootstrap sequence (cold-start snapshot, KW baseline comparison)
4. WP loads `kw-calibration.json`, verifies checksum
...
5. WP reads `kw-meta-audit.md` tail (last 100 entries) to rebuild recent context
6. WP verifies hash chain integrity on `kw-meta-audit.md`
   - If broken: `WP_AUDIT_INTEGRITY_FAILURE` (critical, to user)
7. WP enters observation mode

WP does not inherit runtime state across restarts. Its state is reconstructed
from its persistent files (audit log + calibration JSON), both integrity-checked on load.
...
1. WP starts with seed confidence values from this spec
2. WP creates empty `kw-calibration.json` with initial checksum
3. WP creates `kw-meta-audit.md` with genesis entry
4. WP proceeds to KW bootstrap calibration check (Invariant 2)
...
- WP's first action is to capture a cold-start snapshot — if WP is corrupted, the
  snapshot is wrong, but KW's independent baseline comparison may surface the
  inconsistency from the other direction
- The user can manually verify WP's first audit entries against their own
...
WP writing corrupt data itself — that is the irreducible self-trust boundary.
The user can diff `kw-calibration.json` against `kw-meta-audit.md` entries
to detect WP self-corruption.
...
- On a sampling basis during normal operation:
  ...
  - Every 1,000th append: WP verifies the full chain
- On any user request

If verification fails at any point:

Flag: `WP_AUDIT_INTEGRITY_FAILURE` (critical, to user directly)

WP continues operating but audit chain is marked broken from that point.
All subsequent entries include `"chain_status": "broken_since_{timestamp}"`.
...
- Stamp each KW entry with a `wp_verified: true/false` field
...
- Append-only, not rotated
- WP cannot write to this file (tool-scoping enforced)

WP uses this file for Invariant 6 only. The user uses this file as their
primary audit surface for Orchestrator behavior.
...
Invariant 6 signals (`ORCH_MONITORING_ACTION_*`) bypass batching and go to
the user immediately — the Orchestrator cannot be trusted to relay them.
...
```
KW tools:
  write_file:  open-issues.md only
  read_file:   open-issues.md, pipeline logs

WP tools:
  write_file:  kw-meta-audit.md, kw-calibration.json,
...
Orchestrator tools:
  write_file:  orchestrator-decisions.md (append-only)
               (Orchestrator has broader access for pipeline management,
...
Its primary subject is KW. Its secondary subject is the Orchestrator's effect
on KW. Its non-subject is everything else.
...
| `references/bootstrap-spec.md` | ✅ built | Cold-start snapshot methodology              |
| `references/degraded-modes.md` | ✅ built | Full decision trees for every degraded state |
| `references/enforcement.md`    | ✅ built | Tool-scoping implementation per component    |
...
| `references/canary-protocol.md`       | P0       | Canary injection design, frequency, expected KW response latency, false negative rate calculation | v0.5 leak 6          |
| `references/outcome-resolution.md`    | P1       | How actual outcomes are determined for confidence calibration                                     | v0.4 §7c             |
| `references/escalation-matrix.md`     | P1       | Full severity × flag-type routing table, Invariant 6 direct-to-user routing                       | v0.4 §2, §9          |
| `references/signal-quality.md`        | P1       | Base rate estimation, adaptive FPR thresholds, false negative inference from canary data          | v0.4 §7a, §7b        |
| `references/drift-detection.md`       | P1       | Rolling window parameters, gradual vs. abrupt drift, anti-gaming                                  | v0.4 §13             |
| `references/calibration-spec.md`      | P2       | Confidence recalibration algorithm, seed value justification                                      | v0.4 §4              |
| `references/audit-rotation.md`        | P2       | Hash chain implementation, rotation policy, sampling verification schedule                        | v0.4 §6, v0.5 leak 5 |
| `references/schema-evolution.md`      | P2       | Signal format versioning, backward compatibility                                                  | v0.4 §14             |
| `references/orchestrator-log-spec.md` | P2       | `orchestrator-decisions.md` schema, retention policy, user audit workflow                         | v0.5 leak 1          |
...
| 0.4.0   | Post-stress-test v1. Added: WP liveness, severity-gated batching, degraded mode table, gradual drift detection, content_hash dedup, hash-chained audit rotation, tool-scoped enforcement, schema versioning, Rule 3 bootstrap compliance, statistical plausibility clarification, base-rate-adjusted FPR threshold, 11 reference files scoped.                                                                                                                                                                                     |
| 0.5.0   | Post-stress-test v2. Closed 6 leaks from v0.4.0. Added: Invariant 6 (monitoring stack action audit), `orchestrator-decisions.md`, `kw-calibration.json` checksum + Rule 8, WP deterministic bootstrap, WP restart protocol (file-based state reconstruction), hash chain continuous verification (sampling schedule), canary injection for false negative detection, Invariant 6 bypass batching, who-watches-what table, 4 known limitations declared. New reference: `canary-protocol.md` (P0), `orchestrator-log-spec.md` (P2). |
