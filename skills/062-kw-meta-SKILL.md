...
description: >
  ...
  drift, silence, and rule violations. Use this skill whenever someone
  needs to audit KW behavior, check if the Kernel Witness is drifting,
  review KW's flagging patterns, investigate KW silence, or set up a
  meta-observation layer over any monitoring system. Triggers on phrases
  like "is KW working," "audit the witness," "check KW drift," or any
  concern about the integrity of a monitoring agent.
---
...
`gradual_windows x window_size` steps before triggering (default: 250).
On fast pipelines this may be hours. Tune `window_size` down if the
pipeline runs quickly.

*Self-reported exemption:* `KW_CRITERIA_CHANGE` is written by KW itself.
WP cannot verify the legitimacy of these declarations. WP logs the rate
of `KW_CRITERIA_CHANGE` entries per window. An unusual frequency is
...
If `open-issues.md` is missing or empty when `pipeline-status.md` reads
`active`, WP treats this as a `KW_SILENT` condition and logs accordingly.
...
fact. The Orchestrator already received KW's direct message. WP logs the
violation. Whether the Orchestrator discards the original KW message is
an Orchestrator-level enforcement concern, outside WP's scope.
...
**Silence detection trusts the Orchestrator.** `pipeline-status.md` is
written by the Orchestrator. A compromised or stalled Orchestrator that
writes `idle` suppresses silence alerts. The user reading the log and
noticing an unexpected `idle` status is the fallback.

**Restart resets drift sensitivity.** After a restart, drift detection
operates at reduced sensitivity for up to `window_size` steps (default: 50)
for abrupt drift and `gradual_windows x window_size` steps (default: 250)
for gradual drift. Repeated forced restarts could keep WP permanently in
reduced-sensitivity mode.

**KW controls its own exemptions.** `KW_CRITERIA_CHANGE` declarations are
self-reported by KW. WP logs their frequency but cannot verify their
...
| 0.6.2 | Second stress test. Five fixes, no new mechanisms. (1) Expanded Known Limitations to surface pipeline-status trust dependency, restart sensitivity gap, and KW self-reported exemptions — previously buried in other sections. (2) Reframed relay-after-violation: WP logs the violation; Orchestrator enforcement is outside WP's scope. (3) Added criteria-change frequency logging to make self-reported exemption abuse visible in the audit log. (4) Defined behavior when `open-issues.md` is missing or empty during active pipeline — treated as `KW_SILENT`. (5) Improved skill description for reliable triggering. |
